...
const uint8_t PROGMEM BOX_BITMAP[] = {
  0x7f, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41,  0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41,  0x7f, // EMPTY
  0x7f, 0x61, 0x61, 0x61, 0x51, 0x51, 0x51, 0x49,  0x49, 0x49, 0x45, 0x45, 0x45, 0x43, 0x43, 0x43,  0x7f, // SLASH
  0x7f, 0x43, 0x43, 0x43, 0x45, 0x45, 0x45, 0x49,  0x49, 0x49, 0x51, 0x51, 0x51, 0x61, 0x61, 0x61,  0x7f, // BACK_SLASH
  0x7f, 0x61, 0x61, 0x61, 0x51, 0x51, 0x51, 0x49,  0x49, 0x49, 0x51, 0x51, 0x51, 0x61, 0x61, 0x61,  0x7f, // AND
  0x7f, 0x43, 0x43, 0x43, 0x45, 0x45, 0x45, 0x49,  0x49, 0x49, 0x45, 0x45, 0x45, 0x43, 0x43, 0x43,  0x7f, // OR
};
...
void loop() {
  ...
  *erase = 0;
  ...
  if (arduboy.pressed(A_BUTTON)) {
    box_t b = BOARD[address(cur_x, cur_y + 1)];
    BOARD[address(cur_x, cur_y + 1)] = BOARD[address(cur_x, cur_y + 2)];
    BOARD[address(cur_x, cur_y + 2)] = b;
  }

  if (arduboy.pressed(UP_BUTTON))    cur_y = max(cur_y - 1, 0);
  if (arduboy.pressed(DOWN_BUTTON))  cur_y = min(cur_y + 1, 8);
  if (arduboy.pressed(LEFT_BUTTON))  cur_x = max(cur_x - 1, 0);
  if (arduboy.pressed(RIGHT_BUTTON)) cur_x = min(cur_x + 1, 4);

  arduboy.clear();
  for (int x = 0; x < 5; x++) {
    for (int y = 0; y < 10; y++) {
      arduboy.drawBitmap(16 * x + 24, 6 * y + 3, &BOX_BITMAP[17 * BOARD[address(x, y+1)]], 17, 8, WHITE);
    }
  }
  arduboy.drawBitmap(16 * cur_x + 24, 6 * cur_y + 3, CURSOR_BITMAP, 17, 16, INVERT);
